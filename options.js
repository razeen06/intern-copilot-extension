...
const CHECK_ICON_SVG = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>`;
...
async function renderApprovedSites() {
  ...
  if (userApproved.length === 0) {
    container.innerHTML = `<div class="empty-state">No additional sites enabled yet.</div>`;
    return;
  }
    ...
    .map((script) => {
      ...
      return `
        <div class="site-row">
          ...
          <button class="remove-site-btn" data-origin="${originPattern}" data-id="${script.id}">${TRASH_ICON_SVG}<span>Remove</span></button>
        </div>
      `;
    })
    .join("");
